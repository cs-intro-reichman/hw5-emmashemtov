...
public static void storeGuess(String guess, char[][] guesses, int row) {
    for (int col = 0; col < guess.length(); col++) guesses[row][col] = guess.charAt(col);
}
...
public static void main(String[] args) {
    ...
    // Losing message (safe index)
    if (!won) {
        int lastRow = Math.max(0, attempt - 1);
        printBoard(guesses, results, lastRow);
